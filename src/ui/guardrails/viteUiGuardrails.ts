...
function findMatches(source: string, file: string): GuardrailFinding[] {
  ...
  const addMatches = (pattern: RegExp, rule: string) => {
    ...
    while (match) {
      ...
      findings.push({ file, line, rule, snippet: readLine(source, line) })
      match = pattern.exec(source)
    }
  }

  if (!source.includes(ALLOW_ABSOLUTE_MARKER)) {
    addMatches(CLASSNAME_ABSOLUTE_PATTERN, 'absolute/fixed layout positioning')
  }
