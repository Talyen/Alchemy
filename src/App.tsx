...
import type { CollectionTab, CombatTextAnimationVariant, DisplayMode, ResolutionOption, Screen, UiScale } from "@/features/alchemy/types";
...
export default function App() {
  ...
  const [combatTextAnimationVariant, setCombatTextAnimationVariant] = useState<CombatTextAnimationVariant>("bounce");
        ...
        <div className={`flex h-screen w-screen items-center justify-center overflow-hidden bg-background ${isMobileLandscape ? "mobile-landscape p-0" : "p-4"}`}>
          <div className="relative" style={frameStyle}>
            <div ref={vrStageRef} className="absolute left-0 top-0 overflow-hidden bg-background" style={stageStyle}>
              ...
              {renderedScreen === "menu" ? <MenuScreen onPlay={run.beginRun} hasActiveBattle={run.hasActiveBattle} onCollection={() => run.goToScreen("collection")} onOptions={() => run.goToScreen("options")} onHomestead={() => run.goToScreen("homestead")} onTalents={() => run.goToScreen("talents")} {...(platform.canQuit ? { onQuit: platform.quit } : {})} logoSrc={menuLogo} isMobileLandscape={isMobileLandscape} /> : null}
