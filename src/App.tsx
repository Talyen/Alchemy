...
import type { CollectionTab, ResolutionOption, Screen } from "@/features/alchemy/types";
...
export default function App() {
  ...
  const run = useAlchemyRunController({ discoveredCardIds, setDiscoveredCardIds, setEncounteredEnemyIds, discoveredTrinketIds, setDiscoveredTrinketIds, initialTalentXP: initialSave.talentXP, initialUnlockedTalents: initialSave.unlockedTalents, initialActiveRun: initialSave.activeRun });
    ...
    <div className="flex h-screen w-screen items-center justify-center overflow-hidden bg-background p-4">
      <div className="relative" style={frameStyle}>
        <div className="absolute left-0 top-0 overflow-hidden bg-background" style={stageStyle}>
          ...
          {renderedScreen === "menu" ? <MenuScreen onPlay={run.beginRun} hasActiveBattle={run.hasActiveBattle} onCollection={() => run.goToScreen("collection")} onOptions={() => run.goToScreen("options")} onTalents={() => run.goToScreen("talents")} logoSrc={menuLogo} /> : null}
