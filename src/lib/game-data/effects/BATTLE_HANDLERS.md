...
[`applyCardEffects`](../../battle/effect-handlers/dispatch.ts) is the single entry point exported from `@/lib/battle`. It walks each effect on a card, routes `chance` (via `rollChance` + `getBattleRng`) and `repeat-over-turns` (queue `pendingTurnStartEffects`) before the registry, and otherwise delegates to `applyEffectByKind`.
