...
const STATUS_ANOMALY_METRICS: StatusAnomalyMetric[] = [
  { key: "maxPlayerBlock", label: "Block on Player", read: (s) => s.playerStatuses.block ?? 0 },
  { key: "maxPlayerArmor", label: "Armor on Player", read: (s) => s.playerStatuses.armor ?? 0 },
  { key: "maxPlayerBurn", label: "Burn on Player", read: (s) => s.playerStatuses.burn ?? 0 },
  { key: "maxPlayerPoison", label: "Poison on Player", read: (s) => s.playerStatuses.poison ?? 0 },
  { key: "maxPlayerBleed", label: "Bleed on Player", read: (s) => s.playerStatuses.bleed ?? 0 },
  { key: "maxPlayerFreeze", label: "Freeze on Player", read: (s) => s.playerStatuses.freeze ?? 0 },
  { key: "maxPlayerStun", label: "Stun on Player", read: (s) => s.playerStatuses.stun ?? 0 },
  { key: "maxEnemyBurn", label: "Burn on Enemy", read: (s) => s.enemyStatuses.burn ?? 0 },
  { key: "maxEnemyPoison", label: "Poison on Enemy", read: (s) => s.enemyStatuses.poison ?? 0 },
  { key: "maxEnemyBleed", label: "Bleed on Enemy", read: (s) => s.enemyStatuses.bleed ?? 0 },
  { key: "maxEnemyFreeze", label: "Freeze on Enemy", read: (s) => s.enemyStatuses.freeze ?? 0 },
  { key: "maxEnemyStun", label: "Stun on Enemy", read: (s) => s.enemyStatuses.stun ?? 0 },
