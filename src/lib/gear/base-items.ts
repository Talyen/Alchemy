import type { KeywordId } from "@/lib/game-data";
import { emptyInventory } from "@/lib/homestead/inventory";
import type { MaterialInventory } from "@/lib/homestead/types";
import type { GearRarity, GearSlot } from "./types";

export type GearBaseItemDefinition = {
  id: string;
  displayName: string;
  compatibleSlots: GearSlot[];
  requiresTwoHands: boolean;
  affinityKeywords: KeywordId[];
  availableRarities: readonly GearRarity[];
  salvageByRarity: Record<GearRarity, MaterialInventory>;
};

function salvage(basicIron: number, astralIron: number, astralCrystal = 1): Record<GearRarity, MaterialInventory> {
  return {
    basic: { ...emptyInventory(), iron: basicIron },
    astral: { ...emptyInventory(), iron: astralIron, crystal: astralCrystal },
  };
}

const lightSalvage = salvage(1, 2);
const mediumSalvage = salvage(2, 3);
const heavySalvage = salvage(3, 4);

export const gearBaseItems = {
  "great-axe": {
    id: "great-axe",
    displayName: "Great Axe",
    compatibleSlots: ["main-hand"],
    requiresTwoHands: true,
    affinityKeywords: ["physical", "stun"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: heavySalvage,
  },
  "great-mace": {
    id: "great-mace",
    displayName: "Great Mace",
    compatibleSlots: ["main-hand"],
    requiresTwoHands: true,
    affinityKeywords: ["physical", "stun", "holy"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: heavySalvage,
  },
  "great-sword": {
    id: "great-sword",
    displayName: "Great Sword",
    compatibleSlots: ["main-hand"],
    requiresTwoHands: true,
    affinityKeywords: ["physical", "stun"],
    availableRarities: ["astral"],
    salvageByRarity: heavySalvage,
  },
  "hand-axe": {
    id: "hand-axe",
    displayName: "Hand Axe",
    compatibleSlots: ["main-hand"],
    requiresTwoHands: false,
    affinityKeywords: ["physical"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: mediumSalvage,
  },
  "long-sword": {
    id: "long-sword",
    displayName: "Long Sword",
    compatibleSlots: ["main-hand"],
    requiresTwoHands: false,
    affinityKeywords: ["physical", "stun"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: mediumSalvage,
  },
  "short-sword": {
    id: "short-sword",
    displayName: "Short Sword",
    compatibleSlots: ["main-hand"],
    requiresTwoHands: false,
    affinityKeywords: ["physical"],
    availableRarities: ["basic"],
    salvageByRarity: lightSalvage,
  },
  dagger: {
    id: "dagger",
    displayName: "Dagger",
    compatibleSlots: ["main-hand"],
    requiresTwoHands: false,
    affinityKeywords: ["physical", "bleed", "poison"],
    availableRarities: ["basic"],
    salvageByRarity: lightSalvage,
  },
  mace: {
    id: "mace",
    displayName: "Mace",
    compatibleSlots: ["main-hand"],
    requiresTwoHands: false,
    affinityKeywords: ["physical", "stun", "holy"],
    availableRarities: ["basic"],
    salvageByRarity: mediumSalvage,
  },
  flail: {
    id: "flail",
    displayName: "Flail",
    compatibleSlots: ["main-hand"],
    requiresTwoHands: false,
    affinityKeywords: ["physical", "stun"],
    availableRarities: ["basic"],
    salvageByRarity: mediumSalvage,
  },
  "long-bow": {
    id: "long-bow",
    displayName: "Long Bow",
    compatibleSlots: ["main-hand"],
    requiresTwoHands: false,
    affinityKeywords: ["archery", "physical", "nature"],
    availableRarities: ["basic"],
    salvageByRarity: mediumSalvage,
  },
  "short-bow": {
    id: "short-bow",
    displayName: "Short Bow",
    compatibleSlots: ["main-hand"],
    requiresTwoHands: false,
    affinityKeywords: ["archery", "physical"],
    availableRarities: ["basic"],
    salvageByRarity: lightSalvage,
  },
  "recurve-bow": {
    id: "recurve-bow",
    displayName: "Recurve Bow",
    compatibleSlots: ["main-hand"],
    requiresTwoHands: false,
    affinityKeywords: ["archery", "nature", "physical"],
    availableRarities: ["astral"],
    salvageByRarity: mediumSalvage,
  },
  crossbow: {
    id: "crossbow",
    displayName: "Crossbow",
    compatibleSlots: ["main-hand"],
    requiresTwoHands: true,
    affinityKeywords: ["archery", "physical"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: heavySalvage,
  },
  staff: {
    id: "staff",
    displayName: "Staff",
    compatibleSlots: ["main-hand"],
    requiresTwoHands: true,
    affinityKeywords: ["burn", "freeze", "mana"],
    availableRarities: ["basic"],
    salvageByRarity: mediumSalvage,
  },
  wand: {
    id: "wand",
    displayName: "Wand",
    compatibleSlots: ["main-hand"],
    requiresTwoHands: false,
    affinityKeywords: ["burn", "freeze", "mana"],
    availableRarities: ["astral"],
    salvageByRarity: lightSalvage,
  },
  "leather-shield": {
    id: "leather-shield",
    displayName: "Leather Shield",
    compatibleSlots: ["off-hand"],
    requiresTwoHands: false,
    affinityKeywords: ["block", "armor", "physical"],
    availableRarities: ["basic"],
    salvageByRarity: lightSalvage,
  },
  "plate-shield": {
    id: "plate-shield",
    displayName: "Plate Shield",
    compatibleSlots: ["off-hand"],
    requiresTwoHands: false,
    affinityKeywords: ["block", "armor", "stun"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: heavySalvage,
  },
  quiver: {
    id: "quiver",
    displayName: "Quiver",
    compatibleSlots: ["off-hand"],
    requiresTwoHands: false,
    affinityKeywords: ["archery", "physical"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: lightSalvage,
  },
  spellbook: {
    id: "spellbook",
    displayName: "Spellbook",
    compatibleSlots: ["off-hand"],
    requiresTwoHands: false,
    affinityKeywords: ["burn", "freeze", "holy"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: mediumSalvage,
  },
  "leather-armor": {
    id: "leather-armor",
    displayName: "Leather Armor",
    compatibleSlots: ["body"],
    requiresTwoHands: false,
    affinityKeywords: ["physical", "block"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: mediumSalvage,
  },
  "plate-armor": {
    id: "plate-armor",
    displayName: "Plate Armor",
    compatibleSlots: ["body"],
    requiresTwoHands: false,
    affinityKeywords: ["armor", "block", "stun"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: heavySalvage,
  },
  "leather-hood": {
    id: "leather-hood",
    displayName: "Leather Hood",
    compatibleSlots: ["helm"],
    requiresTwoHands: false,
    affinityKeywords: ["physical", "poison", "bleed"],
    availableRarities: ["basic"],
    salvageByRarity: lightSalvage,
  },
  "leather-helm": {
    id: "leather-helm",
    displayName: "Leather Helm",
    compatibleSlots: ["helm"],
    requiresTwoHands: false,
    affinityKeywords: ["archery", "nature", "physical"],
    availableRarities: ["astral"],
    salvageByRarity: mediumSalvage,
  },
  "plate-helm": {
    id: "plate-helm",
    displayName: "Plate Helm",
    compatibleSlots: ["helm"],
    requiresTwoHands: false,
    affinityKeywords: ["armor", "stun", "block"],
    availableRarities: ["basic"],
    salvageByRarity: heavySalvage,
  },
  "leather-boots": {
    id: "leather-boots",
    displayName: "Leather Boots",
    compatibleSlots: ["boots"],
    requiresTwoHands: false,
    affinityKeywords: ["physical", "archery", "nature"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: lightSalvage,
  },
  "plate-boots": {
    id: "plate-boots",
    displayName: "Plate Boots",
    compatibleSlots: ["boots"],
    requiresTwoHands: false,
    affinityKeywords: ["armor", "stun"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: heavySalvage,
  },
  "leather-gloves": {
    id: "leather-gloves",
    displayName: "Leather Gloves",
    compatibleSlots: ["gloves"],
    requiresTwoHands: false,
    affinityKeywords: ["physical", "bleed"],
    availableRarities: ["basic"],
    salvageByRarity: lightSalvage,
  },
  "leather-belt": {
    id: "leather-belt",
    displayName: "Leather Belt",
    compatibleSlots: ["belt"],
    requiresTwoHands: false,
    affinityKeywords: ["physical", "gold"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: lightSalvage,
  },
  "ruby-ring": {
    id: "ruby-ring",
    displayName: "Ruby Ring",
    compatibleSlots: ["left-ring", "right-ring"],
    requiresTwoHands: false,
    affinityKeywords: ["burn"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: lightSalvage,
  },
  "sapphire-ring": {
    id: "sapphire-ring",
    displayName: "Sapphire Ring",
    compatibleSlots: ["left-ring", "right-ring"],
    requiresTwoHands: false,
    affinityKeywords: ["freeze"],
    availableRarities: ["astral"],
    salvageByRarity: lightSalvage,
  },
  "emerald-ring": {
    id: "emerald-ring",
    displayName: "Emerald Ring",
    compatibleSlots: ["left-ring", "right-ring"],
    requiresTwoHands: false,
    affinityKeywords: ["nature", "poison"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: lightSalvage,
  },
  "topaz-ring": {
    id: "topaz-ring",
    displayName: "Topaz Ring",
    compatibleSlots: ["left-ring", "right-ring"],
    requiresTwoHands: false,
    affinityKeywords: ["holy", "gold"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: lightSalvage,
  },
  "ruby-amulet": {
    id: "ruby-amulet",
    displayName: "Ruby Amulet",
    compatibleSlots: ["amulet"],
    requiresTwoHands: false,
    affinityKeywords: ["burn"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: lightSalvage,
  },
  "sapphire-amulet": {
    id: "sapphire-amulet",
    displayName: "Sapphire Amulet",
    compatibleSlots: ["amulet"],
    requiresTwoHands: false,
    affinityKeywords: ["freeze"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: lightSalvage,
  },
  "emerald-amulet": {
    id: "emerald-amulet",
    displayName: "Emerald Amulet",
    compatibleSlots: ["amulet"],
    requiresTwoHands: false,
    affinityKeywords: ["nature", "poison"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: lightSalvage,
  },
  "topaz-amulet": {
    id: "topaz-amulet",
    displayName: "Topaz Amulet",
    compatibleSlots: ["amulet"],
    requiresTwoHands: false,
    affinityKeywords: ["holy", "gold"],
    availableRarities: ["basic", "astral"],
    salvageByRarity: lightSalvage,
  },
} satisfies Record<string, GearBaseItemDefinition>;

export type GearBaseItemId = keyof typeof gearBaseItems;

export const gearBaseItemList = Object.values(gearBaseItems);
