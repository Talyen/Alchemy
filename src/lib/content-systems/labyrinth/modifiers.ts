...
export function getEnemyModifiersForNodeType(type: "combat" | "elite" | "boss", rng: () => number = Math.random): LabyrinthModifierKind[] {
  const pool = Object.keys(ALL_LABYRINTH_MODIFIERS).filter((k) => !REWARD_MODIFIER_KINDS.has(k as LabyrinthModifierKind)) as LabyrinthModifierKind[];
...
export function getRewardModifiersForNodeType(type: "combat" | "elite" | "boss", rng: () => number = Math.random): LabyrinthModifierKind[] {
  if (type === "boss") return [];
