...
export function MenuScreen({ onPlay, onCollection, onOptions, onTalents, onHomestead, onQuit, logoSrc, hasUnspentTalents = false, hasAffordableHomestead = false, isMobileLandscape = false }: { onPlay: () => void; onCollection: () => void; onOptions: () => void; onTalents: () => void; onHomestead: () => void; onQuit?: () => void; logoSrc: string; hasUnspentTalents?: boolean; hasAffordableHomestead?: boolean; isMobileLandscape?: boolean }) {
  return (
    <div className={`flex h-full w-full flex-col items-center justify-center text-center ${isMobileLandscape ? "gap-2" : "gap-8"}`}>
      ...
      <div className="grid gap-2">
        <Button size={isMobileLandscape ? "sm" : "lg"} className={`stagger-item justify-center gap-2 ${isMobileLandscape ? "w-36 text-xs" : "w-56 text-base"}`} style={{ "--stagger-index": 0 } as CSSProperties} onClick={onPlay}>
          <Swords className={isMobileLandscape ? "h-3.5 w-3.5" : "h-4 w-4"} />Play
        </Button>
