...
export function RunEndObtainedItems({ items }: { items: readonly RunObtainedItem[] }) {
  const visibleItems = items.filter((item) => item.kind === "gear" || trinketById[item.trinketId]);
  const { page, totalPages, pageItems, setPage } = usePaginatedRows(visibleItems, ITEM_PAGE_SIZE, ITEM_COLUMNS);
      ...
      <FadeSlot swapKey={`run-end-items-${page}`} className={paging ? "min-h-[34cqh]" : undefined}>
        <div className="flex w-full flex-wrap items-center justify-center gap-4">
          {pageItems.map((item, index) => {
            ...
            return <RunEndObtainedItemTile key={itemKey} item={item} interactionKey={`run-end-item:${itemKey}`} />;
          })}
        </div>
      </FadeSlot>
    </FlankingPagination>
...
function RunEndObtainedItemTile({ item, interactionKey }: { item: RunObtainedItem; interactionKey: string }) {
  let tile;
  if (item.kind === "gear") {
    tile = <GearTile instance={item.instance} interactionKey={interactionKey} />;
  } else {
    const trinket = trinketById[item.trinketId];
    if (!trinket) return null;
    tile = <TrinketTile trinket={trinket} interactionKey={interactionKey} />;
  }
