...
/** Unified view of an in-progress or resumable run (screen is supplied by the run controller). */
...
export function useRunSessionBattleContext(screen: Screen): RunSessionBattleContext {
  ...
  return useMemo(
    ...
    [screen, battle, activeLabyrinthModifiers],
  );
}
...
export function useRunSessionNavigationSlice(screen: Screen): RunSessionNavigationSlice {
  ...
  return useMemo(
    () => ({
      phase: getRunPhase(screen, hasActiveBattle),
      hasActiveBattle,
      hasActiveRun: session.hasActiveRun,
      labyrinthMap: session.labyrinthMap,
    ...
    [screen, hasActiveBattle, session],
  );
}
