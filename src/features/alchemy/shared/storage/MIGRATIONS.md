...
wire shape. The runtime `session.activity` union replaces parallel visit fields and the separate return-screen field without changing this wire shape. New saves encode the logical activity even before its screen is displayed. Legacy interrupted reward handoffs decode normally; new reward claims persist the advanced bundle atomically with the award, so the claim lock does not decide serialization. No schema bump is required. Menu locations in older snapshots are inferred from surviving run
