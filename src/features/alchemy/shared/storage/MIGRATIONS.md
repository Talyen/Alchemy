...
| `activeRun.activeLabyrinthModifiers` / `activeRun.activeLabyrinthRewardModifiers` | `[]`; expedition-level twists persist outside combat, backfilled from `activeCombat` on old saves | `save-schemas/active-run.ts` + `normalizeActiveRunData`                              |
