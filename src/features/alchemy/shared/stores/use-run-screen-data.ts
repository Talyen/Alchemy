...
const FIELD_GETTERS: { [K in keyof RunScreenData]: (state: State) => RunScreenData[K] } = {
  ...
  runPlayerHealth: (state) => state.progress.run.runPlayerHealth,
  runMaxHealth: (state) => state.progress.run.runMaxHealth,
  runGold: (state) => state.progress.run.runGold,
  runDeck: (state) => state.progress.run.runDeck,
  selectedDifficulty: (state) => state.progress.run.selectedDifficulty,
  talentXP: (state) => state.progress.permanent.talentXP,
  unlockedTalents: (state) => state.progress.permanent.unlockedTalents,
  runTalentXP: (state) => state.progress.run.runTalentXP,
