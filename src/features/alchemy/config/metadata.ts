...
import { BookOpen, CircleOff, Coins, Crosshair, Dices, Droplet, Flame, FlaskConical, Gem, Hammer, Heart, HeartPulse, Leaf, Map, PawPrint, Shield, ShieldAlert, ShieldHalf, Skull, Snowflake, Sparkles, Sun, Swords, WandSparkles, Zap, Trophy } from "lucide-react";

import { alchemistShopBg, campfire, corruptionAltar, eliteEnemyBg, merchantShopBg, mysteryBg, normalEnemyBg, placeholderDestination, theCampaign, theLabyrinth, theWildwoods, type KeywordId } from "@/lib/game-data";
...
export const destinationMeta: Record<Destination, { icon: LucideIcon; className: string; art: string }> = {
  ...
  Corruption: { icon: Dices, className: "bg-black text-red-400 border-red-700/90 hover:bg-red-950/95 hover:text-red-200", art: corruptionAltar },
...
export const gameModeMeta: Record<string, { title: string; description: string; icon: LucideIcon; art: string }> = {
  campaign: { title: "The Campaign", description: "Journey through Act I, Act II, and Act III", icon: Swords, art: theCampaign },
  labyrinth: { title: "The Labyrinth", description: "Descend through a maze of encounters", icon: Map, art: theLabyrinth },
  wildwood: { title: "The Wildwoods", description: "Challenge the wild's most fearsome bosses", icon: PawPrint, art: theWildwoods },
};
