...
import { resolveStunTrigger, applyDamageStatuses, applyPlayerStatusEffect, removeHarmfulPlayerStatuses } from "@/lib/battle/status-effects";
...
describe("resolveStunTrigger", () => {
  ...
  it("resets stun and skips turns when stun exceeds threshold", () => {
    ...
    const state = { ...base, enemyHealth: 30, enemyMaxHealth: 30, enemyStunSkipTurns: 0, enemyStatuses: { ...base.enemyStatuses, stun: 20 } };
    const result = resolveStunTrigger(state);
  ...
  it("does nothing when enemy is dead", () => {
    const state = baseState({ enemyHealth: 0, enemyMaxHealth: 30, enemyStunSkipTurns: 0, enemyStatuses: { ...baseState().enemyStatuses, stun: 20 } });
    const result = resolveStunTrigger(state);
    expect(result).toBe(state);
  });
  ...
  it("skips additional turns with stunDurationExtension", () => {
    const state = baseState({
      enemyHealth: 30, enemyMaxHealth: 30, enemyStunSkipTurns: 0,
  ...
  it("uses stunThresholdReduction to lower threshold", () => {
    const base = baseState();
    const state = { ...base, enemyHealth: 30, enemyMaxHealth: 30, enemyStunSkipTurns: 0,
...
describe("applyDamageStatuses", () => {
  ...
  it("bleed with lifesteal adds to bleedLeech", () => {
    const state = baseState();
    const effect = { kind: "damage" as const, damageType: "bleed" as const, amount: 5, lifesteal: true };
    const result = applyDamageStatuses(state, effect, 5, []);
    expect(result.enemyStatuses.bleedLeech).toBe(10);
  });
  ...
  it("stun adds to stun stack and triggers resolveStunTrigger", () => {
    const base = baseState();
    const state = { ...base, enemyHealth: 30, enemyMaxHealth: 30, enemyStunSkipTurns: 0, enemyStatuses: { ...base.enemyStatuses, stun: 15 } };
  ...
  it("freeze does not trigger on glacial-shell enemies", () => {
    const state = baseState({
      ...
      enemyStatuses: { ...baseState().enemyStatuses, freeze: 15 },
      currentEnemy: {
        id: "ice-golem", title: "Ice Golem", subtitle: "", descriptionLines: [""], art: "",
        enemyType: "normal", traits: [{ id: "glacial-shell", title: "Glacial Shell", description: "Freeze immune" }],
        attackEffects: [],
      },
    });
    const effect = { kind: "damage" as const, damageType: "freeze" as const, amount: 10 };
