...
describe("MysteryEffectBadge", () => {
  it("shows XP amount in tooltip mode", () => {
    render(<MysteryEffectBadge effect={{ kind: "gainXP", keyword: "mana", amount: 8 }} findCard={undefined} findBoon={undefined} tooltip />);
    expect(screen.getByText(/8/)).toBeTruthy();
    expect(screen.getByText(/Mana/)).toBeTruthy();
  });

  it("references MYSTERY_CARD_CHOICES for chooseCard tooltip text", () => {
    render(<MysteryEffectBadge effect={{ kind: "chooseCard" }} findCard={undefined} findBoon={undefined} tooltip />);
  ...
  it("includes keyword tag in chooseCard tooltip text", () => {
    render(
      <MysteryEffectBadge effect={{ kind: "chooseCard", tag: "archery" }} findCard={undefined} findBoon={undefined} tooltip />,
    );
