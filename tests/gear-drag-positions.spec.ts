...
test.describe("Gear drag positions", { ...armory, ...slow }, () => {
  test("drag equipped item back to a specific inventory cell", async ({ page }) => {
    ...
    await expectItemAtCell(page, "Leather Armor", 3, 1);
  });
  ...
  test("drag inventory item to a specific empty cell", async ({ page }) => {
    ...
    await expectItemAtCell(page, "Leather Belt", 5, 2);
  });
  ...
  test("sort button moves items", async ({ page }) => {
    ...
    const armorItem = gearItemLocator(page, "Leather Armor");
    const boxBefore = await armorItem.boundingBox();
    expect(boxBefore).not.toBeNull();

    await sortButton.click();
    // Wait for the sort re-layout instead of sampling a single frame after click.
    await expect
      .poll(async () => {
        const box = await armorItem.boundingBox();
        if (!box) return Number.POSITIVE_INFINITY;
        return Math.max(Math.abs(box.x - boxBefore!.x), Math.abs(box.y - boxBefore!.y));
      })
      .toBeGreaterThan(5);
  });
