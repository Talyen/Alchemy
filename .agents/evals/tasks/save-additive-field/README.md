...
Setup: Branch from `main`, add an optional persisted field with Zod `.default()` (no `CURRENT_SAVE_SCHEMA_VERSION` bump) per `MIGRATIONS.md` single-responsibility rule.
