...
2026-09-14 — Verification discovery now uses command-local `core.fsmonitor=false` and `core.untrackedCache=false` for changed paths, pre-push cleanliness, and repository inventories used by directory expansion and verification receipts. This prevents reliance on cached filesystem discovery without changing saved Git settings. Original 2026-09-10 evidence: a newly written `combat-feedback-sounds.test.ts` was absent from unrestricted `git status --short` despite being neither ignored nor tracked; targeted `git status --short --untracked-files=all -- <path>` found it with both caches enabled. Temporary-repository tests cover warmed caches, new and ignored files, renamed/deleted paths, unusual filenames, and Git failure; live discovery included this change's new tests. Prevention: [verification discovery](../../CONTRIBUTING.md#what-to-run-when-you-change).

2026-09-14 — Required-transparent art is enforced in the manifest and optimizer for the eight resource icons, battle Mana, and both card piles. Source and prepared pixels are checked even on cache hits; replacements are validated before publication, and failures preserve the prior image, manifest, and orphans. Real PNG/WebP fixtures cover opaque RGB/RGBA, painted checkerboards, empty images, valid alpha, and alpha lost during resizing. Only eleven source fingerprints changed; approved image bytes stayed unchanged. Original 2026-09-10 evidence: approved transparent image references produced five opaque PNGs with painted checkerboards; background-only edits repeated the problem, while fresh generations without references produced real alpha in all five. Pixel validation and light/dark visual inspection, including the reference-free fallback, are owned by the [asset workflow](../../Docs/WORKFLOWS-ASSETS.md#resource-and-battle-ui-masters).

2026-09-14 — Audio startup is currently not reproduced; diagnostics improved. Original 2026-09-07 development run `playwright-20260907t212948z-88179-403a0c` timed out both audio tests on Loading; fresh preview run `playwright-20260907t213143z-89849-1adaa1` passed both. Those old reports are no longer present, so the original cause is unconfirmed. Three independent development-server runs now passed both audio journeys (14.9s, 13.1s, 13.0s), and a fresh preview passed both plus the real cold-start journey (9.2s). The existing music journey keeps its 60-second budget. A deliberately blocked main module verified the shared failure digest's failed request, mode, worker count, navigation timing, and Loading snapshot in 1,189 bytes. Pending/failed module evidence shares existing diagnostic bounds; playback and loading behavior remain unchanged. Prevention: [browser diagnostics](../../tests/e2e/README.md#fixtures-and-diagnostics).

2026-09-14 — Affix sweep: existing timeout mitigation retained; assertion overhead reduced. Original 2026-09-06 broad combat-feedback verification twice exceeded the then-5-second affix sweep budget, most recently `check-20260906t230621z-43224-30899b`; the same file passed focused alongside both combat-feedback unit files. The original contention cause remains unconfirmed, and the test-local 15-second ceiling already added on September 12 is retained. Complete scenario coverage now collects violation totals and at most ten examples instead of thousands of repeated matchers; deliberately broken deck pairing and effect isolation still produce useful failures. The sweep took 0.45s focused and 1.30s within the passing 476-file/5,414-test full suite, run separately from browsers. The complete handoff gate, including balance report construction, passed as `check-20260914t071553z-25671-aa423f`. Prevention: [test value and coverage strategy](../../CONTRIBUTING.md#test-value-and-coverage-strategy).

2026-09-14 — `tests/e2e/specs/contiguous-run.spec.ts` flaked on CI due to an immediate unpolled single read of `battle.handCount()` following a successful `expect.poll` assertion, catching a transient zero-hand frame during deal animation/remount. Removed the redundant unpolled read.
...
2026-09-11 — Transition frame sampling found an interrupted tab reveal jumping from 14% to full opacity, nested fades retaining old items under new headings, and inspection chrome appearing before artwork. Shared reversible fades, keyed identity subtrees, and complete modal preparation/exit snapshots now own prevention in [UI motion](../../Docs/UI_MOTION.md#screen-fade-motion). A 100ms prepared-navigation gap also allowed transient inspection opens; [navigation input](../../Docs/BATTLE_CONTROLLERS.md#data-flow) now gates it. Browser focus tracing showed that a control can still compute hidden after its wrapper is visible; dialog focus waits for the actual control. Real-timing tests cover interruption, delayed/failed artwork, reduced motion, and stable exits.

2026-09-10 — A turn-playback frame returned nested Immer proxies from a command; comparison after commit failed with a revoked-proxy error. Resolve from `current(draft.battle.battleState)` before assembling frames. The [run-state contract](../../Docs/RUN_STATE.md#run-state) and resolved-turn regression now require detached, serializable frame results.
...
2026-09-07 — Bonus reward generation expanded to Archery, Wish, and Nature cards, but pending-reward restoration still filtered for summon-Companion effects, losing saved choices. Primary and bonus choices now share card-ID restoration; themed round-trip and interrupted-handoff tests cover the mismatch. Prevention lives in [reward workflows](../../Docs/RUN_WORKFLOWS.md#add-or-change-post-victory-routing-reward_routes).
...
2026-09-04 — Artwork reveal: startup preload completion did not guarantee later mounted images were decoded. Screen and tab fades now wait for mounted artwork, with dimensions reserved for the menu logo; see [UI](../../Docs/UI_MOTION.md#screen-fade-motion). N/A (one-off).
...
| 2026-09-05 | Feedback fade  | Fresh object inputs to `useHeldWhile` looped in Vitest without React Compiler; memoized snapshots and a focused regression test now enforce [UI guidance](../../Docs/UI_MOTION.md#screen-fade-motion). N/A (one-off).                                    |
...
2026-09-06 — Creating the prescribed evaluation worktrees made the main documentation gate report their copied Markdown as unreachable repository documents (check run `check-20260906t185044z-54265-dbf01b`). The following static run also linted those checkouts against the main tree’s rules (`check-20260906t185512z-56427-a6f052`). Documentation and ESLint inventories now exclude isolated worktrees; regressions still detect a real documentation orphan and still lint the active checkout. Prevention lives in the [script catalog](../../scripts/VERIFICATION.md#checks--verification-nesting-order).
...
- 2026-09-03 — Commands: preserved completion callbacks for successful unchanged commands. Added a regression asserting callback result, unchanged root, and no commit notification; clarified the [command contract](../../Docs/RUN_STATE.md#run-state). Changing this behavior would break idempotent navigation.
...
- 2026-09-06 — Run-state review found restoration refreshed current battle Gold but retained stale Gold in a pending turn result, allowing completion to overwrite the shared purse. Synchronization now preserves the result's unapplied Gold change across restore and purse writes. The [run-state contract](../../Docs/RUN_STATE.md#run-state) and [Gold regressions](../../tests/features/alchemy/shared/stores/gold-purse.test.ts) cover parked spending/earnings, repeated saves, and rollback.
...
2026-09-06 — The Trinket authoring workflow pointed to `lib/trinkets.ts` for `TrinketManifest`, which is owned by `src/lib/game-data/trinket-manifest.ts`. Corrected the [workflow](../../Docs/CONTENT_AUTHORING.md#add-a-new-trinket) and documented required description parity rules and regression tests. N/A (one-off stale owner reference).
...
- 2026-09-07 — Armory targeting classified SVG icons and paths as outside controls because its event guard required `HTMLElement`; the currency cursor already accepted `Element`. Targeting now accepts both HTML and SVG descendants and registers listeners without a timer delay. Prevention lives in the [Armory interaction contract](../../Docs/UI_BROWSING.md#armory-crafting-and-salvage) and [targeting regressions](../../tests/features/alchemy/meta/screens/armory/use-armory-targeting-events.test.ts).

- 2026-09-07 — Talent design review assumed enemy Bleed ticks at turn end, but the engine ticks it before the enemy attack; it also discarded enemy-phase draws when dealing the next hand. Bloodrush now names the natural tick, and the next hand preserves those draws. Prevention lives in [talent event rules](../../Docs/TALENT_RULES.md#talent-event-rules) and [status rewards](../../tests/lib/battle/talent-status-rewards.test.ts) and [turn rewards](../../tests/lib/battle/talent-turn-rewards.test.ts).

- 2026-09-07 — Reviewing talent replacements exposed full-hand Returning Flight card loss, Block-break damage after a lethal counter, and Sanguine Overflow consumption by an unselected damage branch. Prevention lives in [turn rewards](../../tests/lib/battle/talent-turn-rewards.test.ts), [defensive reactions](../../tests/lib/battle/enemy-attack-damage.test.ts), and [status rewards](../../tests/lib/battle/talent-status-rewards.test.ts), [attack bonuses](../../Docs/TALENT_RULES.md#attack-bonuses), and [unique item combat semantics](../../Docs/UNIQUE_ITEMS.md#combat-semantics).
...
- 2026-09-09 — Labyrinth's reveal animated `filter`, overriding completed-room grayscale until the animation ended. Opacity-only reveals now preserve the underlying state. The shared artwork gate also waits for images inserted during layout and ignores stale source decodes; Mystery/Victory prompts now fade with their content. [Screen fades](../../Docs/UI_MOTION.md#screen-fade-motion) owns the rules, with delayed-decode and frame-sampled browser regressions.
...
- 2026-09-10: The mid-claim reward roundtrip test expected primary choices to remain claimable even though `finishRewards` awards them before the navigation callback. This hid repeated rewards on reload. The test now uses a real claim before snapshotting; [WORKFLOWS](../../Docs/RUN_WORKFLOWS.md#add-or-change-post-victory-routing-reward_routes) records the committed-primary versus pending-bonus distinction.
...
- 2026-09-10 — The existing game-menu fade canary intermittently left keyboard focus inside the closing panel. The shared overlay capture guard prevented Tab's native default action as well as activation keys. Preserving Tab's default while stopping propagation fixed the guard; the unchanged browser canary passed ten isolated repetitions, and the overlay unit test now protects that distinction. [UI overlay lifecycle](../../Docs/UI_INTERACTION.md#overlay-lifecycle) owns the rule.

- 2026-09-10 — Review found the Battle path navigation paragraph still describing rendered-swap callbacks after navigation moved to synchronous gameplay preparation. Updated [ARCHITECTURE](../../Docs/BATTLE_CONTROLLERS.md#data-flow) to match `createScreenNavigation`: activity commits before the presentation timer, and cancellation does not undo gameplay. Check both subsystem summaries and detailed owner sections when changing lifecycle timing.

- 2026-09-11 — Review reproduced compact static digests that retained only exit footers for documentation and dependency-boundary failures, and game-art discovery that omitted pile/master constraints. Exit detection now leaves unknown diagnostics eligible for bounded excerpts ([CONTRIBUTING](../../CONTRIBUTING.md#static-build-and-ci-policy)); art selection includes the canonical resource and battle UI master section ([Agent discovery](../../Docs/AGENT_DISCOVERY.md#agent-discovery)). Regression coverage exercises both omissions.
...
[Build gates](../../scripts/VERIFICATION.md#checks--verification-nesting-order) and
...
performance verification wording suggested profiling. The [command contract](../../Docs/RUN_STATE.md#command-atomicity),
...
- 2026-09-12 — An instruction-comparison coding trial passed save tests but failed tooling checks because its workspace-write sandbox denied loopback listeners (`EPERM` on `127.0.0.1`). The incomplete trial was excluded and the save comparisons restarted with matching local-network capability after a listener preflight. The [evaluation setup](../evals/README.md#fixed-setup) now requires checking gate capabilities and recording sandbox/network settings before coding trials.

- 2026-09-12 — Battle-stage extraction passed gameplay parity but `talent-effect-invariants.test.ts` reported four unread Talent fields: its source scanner recognizes `talentEffects` and a few aliases, not arbitrary manifest names. Captured hit facts now retain `talentEffects`; the naming constraint is documented in [battle engine invariants](../../Docs/GAME_RULES.md#engine-invariants). No gameplay behavior or invariant assertions changed.

- 2026-09-12 — Single-run End Run initially reset the battle snapshot before the outgoing battle screen unmounted, producing React's empty-image-src warning in `playwright-20260912t211135z-75725-d2dcc9`. Retaining the final snapshot while immediately clearing activity/continuations fixed the player flow (`playwright-20260912t211333z-76222-a5da69`). Prevention: the terminal transition invariant in [ARCHITECTURE](../../Docs/ARCHITECTURE.md) and the lifecycle comment in `run-lifecycle.ts`.
