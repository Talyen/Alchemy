...
- `src/lib/gear/generation.ts` owns Gear offer construction for rewards and equipment shops. Its internal offering pool records each base as ordinary or Unique when selected. Rarity availability is recalculated before each choice; unreserved bases are preferred, ordinary bases may repeat only when a caller requests shelf filling, and a Unique base cannot appear again on that shelf. The public generators keep the source's rarity policy and run RNG sequence.
...
The keyword affixes use the same battle events as cards and talents. Bramblecall checks Thorns before the Nature card resolves; Spitebloom follows successful Thorns retaliation. Gildmend reacts to combat Gold awards, including kill rewards, while Tithebound checks Forge remaining at the kill. Bloodroot and Heartshock require actual Leech restoration and check their conditions before that healing. Starcatcher uses the existing next-card cost reduction after a Wish from an empty hand; Starfall resolves a normal Freeze follow-up. Ashkeeper and Lastlight trigger when the card is Consumed, with Lastlight checking Mana before Consume refunds. Counterplate reacts only to Armor lost from an enemy attack. Rootmender grants one Leech restoration on Nature damage to a Poisoned enemy, including triggered damage, and does not stack another restoration with Sylvan's chance. Venomtrail adds flat damage to direct Poison hits and Poison ticks against Bleeding enemies, including detonations. Scarfeast applies to the hero's Physical card attacks below half Health. Hallowguard requires a Holy card attack that damages enemy Health while the hero has no Block. Hallowbreak increases Holy damage against Stunned enemies.
