...
**Known drift hotspots (check first):**
...
- **Code snippet drift:** search `docs/`, `AGENTS.md`, `CONTRIBUTING.md`, READMEs for fenced `ts`/`tsx` blocks; verify type names and APIs still exist.
...
- **Broken relative links & anchors:** markdown links and `#heading` anchors; verify targets exist.
- **Version / engine staleness:** Node engine, Electron, Playwright, Vitest versions claimed in docs vs `package.json`.
- **Audit tracker residue:** search `docs/Audits/*.md` for embedded run logs or dated status notes.
