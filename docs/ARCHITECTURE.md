...
`Math.random()` is allowed only to create a fresh run seed or presentation-only values that cannot affect gameplay or persisted state. Run-luck helpers live in `@/lib/rng` (the single door; the old `@/lib/run-rng` shim is removed), small math in `@/lib/math`, and class-name/string/id helpers in `@/lib/utils`. Every draw stays in `[0, 1)`; unknown streams, out-of-range draws, empty ranges, and negative sample counts throw in every build. Counters hash as `counter + 1`, so resume continues at the exact next draw. Snapshot-only placeholders always draw zero and must never reach live combat.
...
| Lifecycle                    | `run-session-lifecycle-port.ts`                                                | Public seam over `run-lifecycle.ts` (run + presentation teardown live together)                                          |
...
`run-setup/run/campaign-start.ts` (previously in `shared/run-flow/`).
