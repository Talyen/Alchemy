...
| Gold                                                  | `gameplay-state-store.runProfile.gold`                 | One profile purse; shops, HUD, and battle read/write this field. `startGold` grants once on a new run start. Leftover `activeRun.runGold` on old saves is copied into `gold` on load and omitted from the current wire shape. |
