...
- **Screen transition:** `navigateTo` → `transition` → `assertScreenTransitionAllowed()` → `navigation.screen` → `renderAlchemyScreenRoute()`. Interactive transitions are governed by the exhaustive `ALLOWED_SCREEN_TRANSITIONS` table in `src/lib/routing/screen-transition-policy.ts`; boot restore/hydration intentionally bypasses that policy after save validation. `useScreenTransitions` validates each request before cancelling previous work and scheduling its replacement. Guards run immediately before applying navigation in every mode; rejection discards the callback too. Default navigation waits `NAVIGATION_DELAY_MS` and releases its callback once at the rendered-screen swap, or at timer completion if already at the target screen. Explicitly delayed and immediate transitions invoke their callbacks immediately after setting the screen; `immediate` takes precedence over `delayMs`. Guards are not rechecked at the rendered-screen swap. Cancellation and hook unmount clear both scheduled timers and callbacks awaiting the swap.
...
| Navigation / rewards   | `shell/use-run-flow-engine.ts` (React wiring) + `createRunFlow` / `run-loop/run/run-flow.ts` + `run-loop/run/run-flow-*.ts` + `shell/use-mystery-event-navigation.ts` + `run-loop/navigation/*`                                                                                   |
