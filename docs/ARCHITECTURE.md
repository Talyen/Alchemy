...
Battle reads are data-only. Battle writes use focused draft-first mutators from `run-session-write-port.ts`; event-time calls open one command and existing command recipes pass their draft explicitly. An asynchronous battle transition that changes logical state must persist its continuation in `activeCombat.pendingBattleTransition` in the same commit as its intermediate state. Presentation delays and display overrides may continue after that commit, but a booted battle must never depend on an in-memory promise or timer to become playable. Dodge XP uses the increase in `BattleState.playerDodgeCount` and is awarded through `awardBattleDodgeXP` when the enemy-turn result is first persisted, in that same command; pending-transition playback/resume never repeats the award. `playerDodgeCount` and `dodgeChanceFromDamage` are battle-owned, persist across turns, and start at zero for each new battle. The combat Health-damage boundary requires explicit `hostile` or `self` provenance for Finding Rhythm; Health costs do not award it.
...
| Battle writes                | `run-session-write-port.ts`                                                                                                  | `setBattleState` (purse-synced commit), `setSyncedBattleState` (presentation sync, no purse touch), `initializeActiveBattle` (fresh hydrate), `begin`/`commit`/`clearBattleTransition`; `readBattle()` is data-only    |
