...
Gameplay mutation callers enter the session through `dispatchRunSessionCommand()` from `run-session-facade.ts`. The command boundary currently delegates to the compatibility coordinator in `shared/stores/run-session-transaction.ts`; that implementation detail is intentionally hidden so the lifetime-matched stores can be replaced by an aggregate reducer without changing feature callers. The stores remain mutation owners for this migration, while `useRunSessionCommitStore` publishes one committed projection for React readers and autosave: successful work publishes one revision, while a thrown error restores the pre-transaction stores and leaves the committed projection unchanged. This prevents screen selectors from observing mixed revisions even though the underlying stores are updated sequentially inside the coordinator. Settings and presentation-only state remain separate. Commands are synchronous and must not span an `await`; audio, navigation timers, presentation updates, and other non-rollbackable work use the command's `afterCommit` option or run after the command returns. Nested commands defer their effects until the outer commit, and discard them if any nested work fails. Projection and persistence adapters may still subscribe to the committed coordinator directly; gameplay callers must not.
...
Run domain types live in `run-domain-types.ts` / `run-domain-store.ts` (stores layer only); permanent-profile fields are typed in `run-state-init.ts`. Composed progress/session/battle views and adapters live in `run-store-views.ts`. Exact screen display contracts live in `run-screen-data.ts` (`RunScreenDataByScreen`); screen-specific React selectors live in `use-run-screen-data.ts` — do not recreate a broad all-screen bag or parallel flatten twin. The committed React/persistence projection lives in `run-session-transaction.ts`; it is the only cross-store snapshot exposed to React readers. Gameplay mutations use the command boundary in `run-session-command.ts`; only projection/persistence adapters and that command implementation should depend on the low-level coordinator. Active-run field init and flatten helpers live in `shared/stores/run-state-init.ts` (`pickActiveRunSessionCoreFields` is the shared core block for controller + session run-slice reads). Fresh-run snapshots live in `shared/run-flow/run-start.ts` (consumed by run-setup and stores). Feature code imports `useRunAdapter`, `useTalentAdapter`, commands, reads, writes, and lifecycle via `run-session-facade`.
