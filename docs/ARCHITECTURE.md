...
Every gameplay cross-store mutation is wrapped in `runSessionTransaction()` from `shared/stores/run-session-transaction.ts`. The lifetime-matched stores remain mutation owners, while `useRunSessionCommitStore` publishes one committed projection for React readers and autosave: successful work publishes one revision, while a thrown error restores the pre-transaction stores and leaves the committed projection unchanged. This prevents screen selectors from observing mixed revisions even though the underlying stores are updated sequentially inside the transaction. Settings and presentation-only state remain separate. Transactions are synchronous and must not span an `await`; audio, navigation timers, presentation updates, and other non-rollbackable work use the transaction's `afterCommit` option or run after the transaction returns. Nested transactions defer their effects until the outer commit, and discard them if any nested work fails.
...
| `snapshotRun(screen?)`            | Read the committed run projection → `ActiveRunData` |
...
Run-loop / run-setup / shell read gear through `shared/stores/gear-read-port.ts` (`readGearManifestForCharacter`, `readHasAnyOwnedGear`, …) instead of reaching into the full store API. Mutations stay on `gear-store` / the armory controller; armory mutations that also adjust active-run health run inside `runSessionTransaction()` so gear and run HP publish together. Autosave observes them through the committed gameplay-session signal.
