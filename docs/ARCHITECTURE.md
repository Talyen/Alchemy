...
| `session`        | One active activity/visit, rewards, labyrinth progress, pending selections, and run-flow claims                                                                                            | Transient per live run; shops, mystery visits, and corruption results persist on `ActiveRunData` via the resume codec |
...
> `runProfile` vs `profile`: one purse lives in `runProfile.gold` alongside homestead and talent progression; `profile` holds Collection discoveries and completion/unlock records — never gameplay currency.

Cross-concern writes go through `run-session-write-port.ts`. Multi-concern lifecycle orchestration is exposed through `run-session-lifecycle-port.ts`. Gear is the sanctioned second write-port: `gear-session-command.ts` (`dispatchGearMutationWithRunHealthSync` / `mutateGearWithRunHealthSync` / `dispatchGearSalvageWithMaterialGrant`) composes gear, profile discovery, homestead materials, and live-run rebinding inside one command. Feature-facing reads (`run-reads`, and `profile-store` / `gear-store` slices) are data-only; command-backed write ports own every gameplay mutation. React orchestration uses narrow hooks from `run-reads.ts`; screens use exact screen-data hooks (battle display via `useBattleScreenRouteData`).

Gameplay mutation callers enter through `dispatchRunSessionCommand()` from `run-session-command.ts`. The command boundary opens one Immer draft of the authoritative aggregate, replaces the aggregate root with one incremented revision when state changes, and discards the draft on failure. Successful commands run `afterCommit` exactly once, including no-op assignments; unchanged commands preserve the root and revision and emit no commit signal. Failed commands do not run `afterCommit`. React selectors and autosave subscribe to that same root. Settings and presentation-only state remain separate. Commands are synchronous and must not span an `await`; audio, navigation timers, presentation updates, and other non-rollbackable work use `afterCommit` or run after the command returns. Draft mutators receive the draft explicitly and compose inside one command; a command body must not call another command. Transactional checks that guard a write (shop gold, refresh counts, purchased slots) read from that same draft rather than a committed read port. Persistence adapters may subscribe to the aggregate commit signal directly; gameplay callers must not.

Command result types reject Promises and Promise-like results, including unions with asynchronous results, through the command factory and gear mutation wrappers. The runtime boundary also rejects thenables inside the recipe before publication, preserving the original root, revision, and RNG counters and skipping `afterCommit`. Rejected asynchronous continuations are observed to avoid an additional unhandled rejection; the boundary cannot cancel external work already started by an invalid callback. The command guard is released before `afterCommit`, so an effect may dispatch a separate command. If an effect throws, its error propagates and the already committed state remains; post-commit errors do not roll back gameplay.

Battle reads expose serializable `BattleSnapshot` values. Opening draws and card plays commit before presentation. `commitEndTurn` resolves the turn and its Companion follow-up with `resolveBattleTurn`, awards Dodge XP, and publishes the result atomically before discard or enemy animation. Playback consumes detached `BattleTurnFrame` values and updates only `battle-presentation-store.displayedBattle`; cancellation or failed animation cannot roll back or advance gameplay. Create detached frames from `current(draft.battle.battleState)` so no revoked Immer proxy can escape the command. `activeCombat.pendingBattleTransition` remains a supported legacy save contract: resume consumes its precomputed result once and resolves any remaining logical work without replaying prior rolls or XP. `playerDodgeCount` and `dodgeChanceFromDamage` are battle-owned, start at zero, and persist across turns. The Health-damage boundary requires explicit `hostile` or `self` provenance for Finding Rhythm; Health costs do not award it.
...
| Gold                                                  | `gameplay-state-store.runProfile.gold`               | One profile purse; live ports, screen data, and session slices read it as `gold`. `startGold` grants once on a new run start. Leftover `activeRun.runGold` on old saves is copied into the purse on load and omitted from the current wire shape. |
...
| Rewards, shops, labyrinth, mystery visits, corruption | `gameplay-state-store.session`                       | Shops and corruption persist through the resume codec; mystery visits persist only while `currentScreen` is mystery. All reward surfaces (Campaign, Labyrinth, and Wildwood) persist through `activeRun.interruptedFlow`.                         |
| Current `Screen`                                      | `gameplay-state-store.run.navigation`                | `useActiveRunScreenValue()`                                                                                                                                                                                                                       |
| Combat snapshot                                       | `gameplay-state-store.battle`                        | Synced during battle                                                                                                                                                                                                                              |
| Battle VFX                                            | `run-loop/battle/battle-presentation-store.ts`       | Not persisted                                                                                                                                                                                                                                     |
| Lifecycle                                             | `run-session-lifecycle-port.ts` → `run-lifecycle.ts` | Restore, snapshot, teardown, battle sync (incl. presentation teardown listeners + battle UI clearing)                                                                                                                                             |

### Persistence API

| API                                    | Role                                                                                                                                                                                                                                                                                                                                                                                      |
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `encodeRunResumeSnapshot(source)`      | **Encode** assembler (`RunSession` → `ActiveRunData`). Private helper: `encodeActiveRunFromSession`. Activity-owned shops via `encodePersistedShops`; current reward/destination bundles via `encode-interrupted-flow.ts`. Legacy screen inference is retained for decoding and an uninitialized activity. Hydrate **parse** uses `toActiveRunData` in `lib/active-run-session/parse.ts`. |
...
`shared/storage/save-candidates.ts` owns save parsing, validation, and future-version protection (deterministic `evaluateSaveCandidates`, for testability); `shared/storage/io.ts` collects candidates, applies the write-disable policy, and owns write serialization. Both delegate raw persistence to one `SaveBackend` configured during bootstrap. `initializeSteam()` returns an explicit `cloudSyncEnabled` capability; it does not mutate shared platform state. Candidate order, Steam Cloud as a one-way mirror, and wipe/protect behavior: [MIGRATIONS.md § Public save contract](../src/features/alchemy/shared/storage/MIGRATIONS.md#public-save-contract).
...
`screen-navigation.ts` runs navigation preparation synchronously, then schedules only the visible screen. Cancelling a fade or unmounting React cannot discard gameplay preparation. Activity route data retains its outgoing snapshot for the fade. Battle outcome handlers are constructed before the battle controller and passed directly; no ref-backed late binding is needed. `shell/run-flow-engine.ts` composes framework-independent command factories, and `use-run-flow-engine.ts` provides only React lifetime and display reads.
...
| Kind                         | Module                                                                                                                       | Role                                                                                                                                                                                                                   |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Reads (imperative)           | `run-reads.ts`                                                                                                               | Data-only committed reads (`readActiveRun()`, `readRunProfile()`, `readRunSession()`, `readBattle()`, `readShopFirstPurchaseUsed()`, …)                                                                                |
...
| Reads (React, orchestration) | `run-reads.ts`                                                                                                               | `useRunSessionNavigationSlice`; `useRunSessionBattleContext` is a display composable, not a shell command source                                                                                                       |
| Reads (React, meta/setup)    | `run-reads.ts`                                                                                                               | Homestead, talent, draft, and content-navigation slices                                                                                                                                                                |
| React action selectors       | `settings-store.ts` (`useSettingsActions`, `useAppSettings`) + module-level `createRunSessionCommand` bindings at call sites | Settings for App chrome; collection/homestead commands are created beside their routes                                                                                                                                 |
| Writes                       | `run-session-write-port.ts` + `dispatchRunSessionCommand`                                                                    | Draft mutators; first argument is `GameplayDraft`. `set*` accepts a value or `(prev) => next` updater. `setHasActiveRun` ships through the barrel — never deep-import a `write-port-*` module outside `shared/stores/` |
| Battle writes                | `run-session-write-port.ts`                                                                                                  | `setBattleState` (purse-synced commit), `setSyncedBattleState` (snapshot replacement, no purse touch), `initializeActiveBattle` (fresh hydrate), `commitBattleTransition`; `readBattle()` is data-only                 |
| Lifecycle                    | `run-session-lifecycle-port.ts`                                                                                              | Public seam over `run-lifecycle.ts` (run + presentation teardown live together). `finalizeRunEndSession` preserves run progress for the recap screens; `teardownRun` fully resets                                      |
...
Screen route → routeCommands.battle (props) → BattleScreen
           → useAlchemyRunController → useBattleController (draft-sourced battle commands)
            → run-loop/battle/* → run-reads / run-session-command → lib/battle
...
| Navigation / rewards   | `shell/use-run-flow-engine.ts` (React wiring) + `createRunFlow` / `run-loop/run/run-flow.ts` + `run-loop/run/run-flow-*.ts` + `run-loop/navigation/mystery-event-navigation.ts` + `run-loop/navigation/*`                                                                            |
| Content-system entry   | `run-setup/run/content-system-navigation.ts` → `run-start-command.ts`; campaign/labyrinth Wildcard draft and novice helpers use `shared/run-flow/starter-draft.ts` + `shared/run-flow/campaign-start.ts`; Wildwood post-entry flow stays in `run-loop/run/wildwood-gauntlet-flow.ts` |
| Run-flow shell actions | `run-loop/run/run-flow.ts` (`RunFlowShellActions`, assembled in `shell/use-run-flow-engine.ts`); destination routers use `DestinationRouteDeps`; reward routing uses `RewardRouteDeps` in `run-flow-rewards.ts`                                                                      |
| Run-flow contracts     | `run-loop/run/run-flow.ts`, `run-destination-handlers.ts` (`DestinationRouteDeps`), `run-flow-rewards.ts` (`RewardRouteDeps`)                                                                                                                                                        |
...
| Session reads/writes   | `shared/stores/run-reads.ts`, `run-session-write-port.ts`, `run-session-command.ts`                                                                                                                                                                                                  |
| Screen routing         | `lib/routing/screen-transition-policy.ts`, `shell/use-screen-transitions.ts`, `useActiveRunScreenValue()`                                                                                                                                                                            |

## Shop commands

`create-shop-actions.ts` is composition only. Each shop's initialization, purchases, services, refreshes, and live price selectors belong to its matching `*-shop-commands.ts` module, sharing purchase and live refresh-price helpers in `shop-commands-core.ts`. Pure shelf samplers live in `shop-state-init.ts`. Each shop supplies a typed shelf assignment to the single `refreshShopOfferings` draft recipe, which owns charging, refresh-count decrement, and purchased-slot reset. Apply offering modifiers before the recipe stores and returns the final items. Draft recipes live in `shop-transactions.ts`; `runShopTransaction` / `commitShopInitialize` are the dispatch seam. Nested `dispatchRunSessionCommand` is illegal — gear HP-sync inside an open shop command uses `mutateGearWithRunHealthSync` ([ARMORY.md § Write paths](./ARMORY.md#write-paths)). Kind `"merchant"` is the player-facing **Card Shop** screen. Only the active visit is encoded by `encodePersistedShops`; stale commands for another shop are rejected by `runShopTransaction`. How-to: [WORKFLOWS § Change a shop](./WORKFLOWS.md#change-a-shop).
...
| Concern                    | Module                                                                                                                                                                                                |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Aggregate fields + actions | `gameplay-state-store.ts` (`GameplayState`: `run` / `session` / `battle` / `runProfile` / `profile` / `gear`)                                                                                         |
| Imperative reads           | `run-reads.ts`                                                                                                                                                                                        |
| Draft mutators             | `run-session-write-port.ts` (re-exports the topical `write-port-*.ts` owners); field setters are created beside their owning domain                                                                   |
| React reads                | `run-reads.ts` (canonical hooks and port field picks)                                                                                                                                                 |
| Initial state              | `run-state-init.ts` (progress/permanent + `ACTIVE_RUN_PROGRESS_KEYS`) and `run-domain-types.ts` (session/battle/run-data); `profile-store-types.ts` / `gear-store-initial-state.ts` own their domains |
