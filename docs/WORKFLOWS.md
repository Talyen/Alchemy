...
| 2. Define the talent (`id`, `keywordId`, name, description, effects, `icon` Lucide export name) | matching `src/lib/game-data/talents/pool/{keyword}.ts` — already spread into `talentPool`. Register the icon in `src/features/alchemy/shared/config/talent-icons.ts` |
