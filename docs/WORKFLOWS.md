...
chambers in five rows spanning five columns, with six variants per chamber count.
A main route zigzags horizontally down the floor, with one or two optional
pockets of one or two chambers. Local loops may attach to consecutive main-route
chambers; long alternate paths and mandatory loops are excluded. Generation
bounds are tighter than save bounds: existing floors through row 8 still load,
and saved coordinates now also accept the fifth column. Actual adjacency defines connectivity,
...
chamber on the current floor is reachable. Prior floors cannot be entered.
Defeating the boss marks it completed; selecting the cleared boss and choosing
Descend advances once, even when optional chambers remain. The player may finish
those rooms before descending. Generation happens only on descent and reuses an
already-generated next floor when present in a saved map. Existing saved floors
are never regenerated by layout changes.

`currentNodeId` records the settled location when a chamber is completed. Pending
travel continues to use `labyrinthPendingNode`; inspecting a chamber does not
change either field. At a new floor or in an older save without location, the
nullable location represents the floor entrance. Invalid or noncompleted saved
location references fall back to the entrance without discarding the map.
Location and pending encounters survive save/resume through the existing codec.
A legacy pending encounter on an earlier floor can finish without replaying
rewards or moving the settled location back from the current floor.
