...
- [x] Phase 2b — Content: effect registry single-source `BATTLE_CARD_EFFECT_KINDS` derived from `TEMPLATE_EFFECT_DEFINITIONS` in `src/lib/game-data/effects/registry.ts`, builder collapse via `effectsCard` generic + `CONSUME_DESCRIPTION_LINE` helper (remaining bespoke prose kept), manifest grouping via `talent-pool-definitions.ts`
