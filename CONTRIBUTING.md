...
Path-filtered jobs (`assets`, `save-gate`, `desktop-build`, `electron-e2e`) are gated by the `changes` job (`dorny/paths-filter`); on `workflow_dispatch` they always run. The `ci-ok` job aggregates every CI job into a single status check — required to **merge PRs** into `main`, not a required **push** gate on `main`. Shared job setup (Node + `npm ci`) lives in the composite action `.github/actions/setup`. Nightly failures open or update a GitHub issue labeled `nightly-failure`. CI fixer Tier B escalations use label `ci-autofix-failed` (deduped; see [docs/CI-FIXER.md](./docs/CI-FIXER.md)).
